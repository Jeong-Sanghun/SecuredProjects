...
public class TwentySecondMemoryManager : MemorySceneManagerParent
{
    [SerializeField]
    ...
    {
        base.Start();
        dialogBundle = jsonManager.ResourceDataLoad<DialogBundle>("ThirdChapter11");
        dialogBundle.SetCharacterEnum();

        nowScene = SceneName.MemoryFriendRoom4;
    ...
    public override void TriggerEnter(string triggerName)
    {
        for (int i = 0; i < nowActionList.Count; i++)
        {
            ...
            if (triggerName.Contains("Target1"))
            {
                ...
                StartCoroutine(SceneEndCoroutine(SceneName.MemoryStore3));
            }
        }
    }
