...
public class TwentySeventhMemoryManager :MemorySceneManagerParent
{
    [SerializeField]
    ...
    {
        base.Start();
        dialogBundle = jsonManager.ResourceDataLoad<DialogBundle>("ThirdChapter16");
        dialogBundle.SetCharacterEnum();

        nowScene = SceneName.MemoryTeacherRoom1;
    ...
    public override void TriggerEnter(string triggerName)
    {
        for (int i = 0; i < nowActionList.Count; i++)
        {
            ...
            if (triggerName.Contains("Target1"))
            {
                ...
                StartCoroutine(SceneEndCoroutine(SceneName.MemoryHallway3));
            }
        }
    }
